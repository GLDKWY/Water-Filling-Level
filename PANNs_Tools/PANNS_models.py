...
from PANNS_utils import do_mixup, interpolate, pad_framewise_output
...
class Wavegram_Logmel_Cnn14(nn.Module):
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        a1 = self.pre_block1(a1, pool_size=4)
        a1 = self.pre_block2(a1, pool_size=4)
        a1 = self.pre_block3(a1, pool_size=4)  # (1, 128, 50)
        a1 = a1.reshape((a1.shape[0], -1, 32, a1.shape[-1])).transpose(2, 3)  # (1, 4, 50, 32)
        a1 = self.pre_block4(a1, pool_size=(2, 1))  # (1, 64, 25, 32)

        # Log mel spectrogram
        x = self.spectrogram_extractor(input)  # (batch_size, 1, time_steps, freq_bins)
        x = self.logmel_extractor(x)  # (batch_size, 1, time_steps, mel_bins) (1, 1, 51, 64)
        ...
        x = self.conv_block1(x, pool_size=(2, 2), pool_type='avg')  # (1, 64, 25, 32)
